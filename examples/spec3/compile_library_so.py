...
``adc.compile_library("my_numerics.so", objects=[...], backend="production", emit=True)``
...
symbols, plus the ``adc_brick_manifest()`` JSON the external-brick loader reads.

``adc.read_library_manifest("my_numerics.so")`` dlopens the ``.so`` and reads that descriptor
back, with a HARD ABI / Kokkos guard (a library compiled against another toolchain is rejected,
never silently loaded). ``adc.compile_problem(..., libraries=["my_numerics.so"])`` reads +
validates it (the consume path).

This script always prints the manifest (pure Python). The real emit + compile + read-back is
guarded on Kokkos being visible (``export ADC_KOKKOS_ROOT=/path/to/kokkos``); without it the
compile section is skipped honestly rather than faked.

Run: python3 examples/spec3/compile_library_so.py
     ADC_KOKKOS_ROOT=/opt/homebrew/anaconda3 python3 examples/spec3/compile_library_so.py
"""
import os
import tempfile
...
def build_manifest():
    ...
    return adc.compile_library("my_numerics.so", objects=objects, backend="production")
...
def main():
    ...
    #    compiled with Kokkos; point ADC_KOKKOS_ROOT at an installed Kokkos.
    print("\n== compile a real library .so (needs Kokkos) ==")
    with tempfile.TemporaryDirectory() as tmp:
        so = os.path.join(tmp, "my_numerics.so")
        try:
            compiled = adc.compile_library("my_numerics.so", objects=[
                lib.solvers.GMRES(), lib.riemann.HLLC()],
                backend="production", emit=True, so_path=so)
        except (RuntimeError, OSError) as exc:
            print("compile requires Kokkos + a matching toolchain (set ADC_KOKKOS_ROOT):")
        ...
        assert back.abi_key == adc.abi_key(), "the .so ABI key matches the loaded module"

        # 4) The library .so is ALSO a self-describing external-brick .so: load_cpp_library reads
        #    its adc_brick_manifest() JSON and registers the ids in the in-process catalog.
