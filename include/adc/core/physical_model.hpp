...
///
/// Aux INVARIANT: every PhysicalModel receives an adc::Aux (phi, grad phi, extra fields).
...
///
/// device INVARIANT: the concept methods (flux, source, ...) must be ADC_HD if
...
//
// Aux contract (slice, see milestone 4): the auxiliary is FIXED to adc::Aux (phi, grad
// phi). This is what load_aux builds and all the spatial operator provides;
// the concept therefore requires it explicitly (M::Aux == adc::Aux) rather than letting
...
///
/// Requires: State, Aux == adc::Aux, n_vars, flux(u,a,dir), max_wave_speed(u,a,dir),
/// source(u,a), elliptic_rhs(u). All these methods must be ADC_HD if called
...
//
// GPU/MPI PRODUCTION: like flux/source, these methods must be ADC_HD (evaluated in
