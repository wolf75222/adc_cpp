...
/// @file
/// @brief 0D kernel of the linearized Langmuir mode (LangmuirMode): IMEX VALIDATION/REFERENCE brick.
///
/// Canonical home of the validation/reference bricks (ADC-329): kept OUT of the production brick
/// surface and exposed under namespace pops::validation. Kept as an analytic example of the IMEX
/// scheme (explicit_step / implicit_solve): the plasma term omega_p^2 is stiff (handled implicitly,
/// A-stable), while the acoustic correction c_s^2 k^2 stays explicit. Not used by adc_cases as of
/// 2026-06-06. The legacy path <pops/physics/langmuir.hpp> still works through a compat forwarder.
...
struct LangmuirMode {
   ...
   */
  POPS_HD void explicit_step(Real& a, Real& b, Real dt) const { b += dt * (-cs2k2 * a); }
