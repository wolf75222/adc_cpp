...
#include <pops/numerics/elliptic/poisson/poisson_operator.hpp>  // apply_laplacian (Lap phi^n)
#include <pops/numerics/linalg/lorentz_eliminator.hpp>         // closed 2x2 B^{-1}
...
/// The same centered divergence stencil is exposed standalone as pops::apply_divergence
...
struct NegateKernel {
  ConstArray4 src;
  Array4 dst;
  POPS_HD void operator()(int i, int j) const { dst(i, j, 0) = -src(i, j, 0); }
};
