...
static double f_tensor(double r, double th, int m, double arr, double art, double atr, double att) {
  const double rad = 0.0 + 0.5 / r + (Hpp(r) + Hp(r) / r) * std::cos(m * th);     // S''+S'/r + (H''+H'/r) cos
...
static DistributionMapping rr_dm(const BoxArray& ba) { return DistributionMapping(ba.size(), n_ranks()); }
...
static void fill_const_local(MultiFab& mf, double val) {
  for (int li = 0; li < mf.local_size(); ++li) {
    ...
    for (int j = vb.lo[1]; j <= vb.hi[1]; ++j)
      for (int i = vb.lo[0]; i <= vb.hi[0]; ++i) a(i, j, 0) = val;
  }
}
...
int main(int argc, char** argv) {
  ...
  if (me == 0)
    std::printf("=== PARITE MULTI-BOX (plusieurs boites par rang) du Schur polaire tensoriel ===\n"
                "Anneau r in [%.2f, %.2f], theta in [0, 2pi). Cible : plusieurs boites LOCALES (np=1).\n",
                kRmin, kRmax);
  auto chk = [&](bool c, const char* w) {
    if (!c) { if (me == 0) std::printf("  ECHEC %s\n", w); ok = false; }
  };
...
#define SAY(...) do { if (me == 0) std::printf(__VA_ARGS__); } while (0)
  ...
  SAY("\n--- (A) Tenseur croise : theta-split (8 boites) vs mono-box [RadialLine] ---\n");
  {
    ...
    PolarKrylovResult kr_ref =
        solve_mb(g, ba_mono, Problem::Dirichlet, m, arr, art, atr, att, PolarPrecond::RadialLine, phi_ref);
    ...
    PolarKrylovResult kr8 =
        solve_mb(g, ba8, Problem::Dirichlet, m, arr, art, atr, att, PolarPrecond::RadialLine, phi8);
  ...
  SAY("\n--- (B) Pavage 2D (coupe r ET theta, 4 boites) vs mono-box [Jacobi] ---\n");
  {
    ...
    MultiFab phi_ref(ba_mono, rr_dm(ba_mono), 1, 1);
    PolarKrylovResult kr_ref =
        solve_mb(g, ba_mono, Problem::Dirichlet, m, arr, art, atr, att, PolarPrecond::Jacobi, phi_ref);
    const double err_ref = err_l2(phi_ref, g, Problem::Dirichlet, m);
  ...
  SAY("\n--- (D) Neumann (jauge) : theta-split (8 boites) vs mono-box [RadialLine] ---\n");
  {
    ...
    MultiFab phi_ref(ba_mono, rr_dm(ba_mono), 1, 1);
    PolarKrylovResult kr_ref =
        solve_mb(g, ba_mono, Problem::Neumann, mN, 1.0, 0.0, 0.0, 1.0, PolarPrecond::RadialLine, phi_ref);
  ...
  SAY("\n=== VERDICT : %s ===\n", ok ? "SUCCESS" : "ECHEC");
  if (ok) SAY("OK test_polar_schur_multibox\n");
#if defined(ADC_HAS_KOKKOS)
  }
  Kokkos::finalize();
#endif
