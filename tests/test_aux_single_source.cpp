// Source UNIQUE de la disposition aux (X-macro ADC_AUX_FIELDS, adc/core/state.hpp). Ce test
...
//
//   (A) CHEMIN DEVICE (load_aux<NComp>) : pour CHAQUE champ de ADC_AUX_FIELDS, on remplit la
//       SEULE composante a son indice et on verifie que load_aux la depose dans le BON membre
//       de Aux. La boucle est elle-meme generee par ADC_AUX_FIELDS -> un nouveau champ ajoute a
...
static constexpr int kNExtra = [] {
  int n = 0;
#define ADC_AUX_COUNT(name, idx) ++n;
  ADC_AUX_FIELDS(ADC_AUX_COUNT)
#undef ADC_AUX_COUNT
  return n;
}();
...
static constexpr int kFullWidth = [] {
  int w = kAuxBaseComps;
#define ADC_AUX_WIDTH(name, idx) w = (idx) + 1 > w ? (idx) + 1 : w;
  ADC_AUX_FIELDS(ADC_AUX_WIDTH)
#undef ADC_AUX_WIDTH
  return w;
}();
...
#undef ADC_AUX_CHECK_READ
  ...
  // python/system.cpp, GENEREE depuis ADC_AUX_FIELDS, et compare champ par champ a load_aux.
  {
