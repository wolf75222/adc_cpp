...
int main() {
  ...
  const int nc = 1;
  RegLite g;
  g.I0 = 2; g.I1 = 5; g.J0 = 2; g.J1 = 5;
  const int nJ = g.J1 - g.J0 + 1, nI = g.I1 - g.I0 + 1;
  g.cL.assign(nJ * nc, Real(1)); g.cR.assign(nJ * nc, Real(2));
  g.cB.assign(nI * nc, Real(3)); g.cT.assign(nI * nc, Real(4));
  g.fL.assign(nJ * nc, Real(10)); g.fR.assign(nJ * nc, Real(20));
  g.fB.assign(nI * nc, Real(30)); g.fT.assign(nI * nc, Real(40));
  ...
  if (fails == 0) std::printf("OK test_cf_interface\n");
  return fails == 0 ? 0 : 1;
}
