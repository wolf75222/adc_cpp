...
// @adc.lib.solver IR-DSL lowers a CUSTOM Richardson solver to a self-contained C++ kernel
// (adc.lib.generate_solver_cpp), emitted by scripts/gen_solver_kernel.py into the header included
// below. This test COMPILES that generated kernel against the real adc::adc runtime and RUNS it on a
// known linear system, proving the lowering is not merely emitted but executes -- and matches the
// native adc::richardson_solve (the same matrix-free Krylov backend the DSL maps onto).
//
...
//   generated solver and the native richardson_solve (which wraps it in an adc::ApplyFn).
//
...
// (adc::dot -> all_reduce_sum, the identity in serial).

#include "adc_generated_solver.hpp"  // GENERATED by scripts/gen_solver_kernel.py: richardson_gen_solve

#include <adc/numerics/elliptic/linear/generic_krylov.hpp>  // adc::richardson_solve (reference)
...
// native richardson_solve wraps the same object in an adc::ApplyFn. The lap scratch is captured once.
struct HelmholtzOp {
  Geometry geom;
  BCRec bc;
  MultiFab* lap_tmp;
...
int main() {
  ...
  // --- Run the GENERATED custom solver (the lowered @adc.lib.solver IR) ---
  ...
  const Real bnorm = std::sqrt(adc::dot(rhs, rhs));
