...
- **MPI** distributes subdomains across ranks (one GPU per rank in GPU mode). Everything goes
  ...
  [`include/pops/parallel/comm.hpp`](https://github.com/wolf75222/adc_cpp/blob/master/include/pops/parallel/comm.hpp). Without
  `POPS_HAS_MPI`, these functions return rank 0 / 1 rank: the code is serial by
  construction.
- **Kokkos** parallelizes the local compute and abstracts the hardware through its `ExecutionSpace`:
  ...
  [`include/pops/mesh/execution/for_each.hpp`](https://github.com/wolf75222/adc_cpp/blob/master/include/pops/mesh/execution/for_each.hpp), which switches
