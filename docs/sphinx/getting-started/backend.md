...
`pops.abi_key()` (compiler + C++ standard + header signature), which serves the native DSL,
...
describes the `_pops` module as CI builds it, in Kokkos Serial; it is not a limit of the
Python. The module links the same `pops::pops` as the tests (cf. `python/CMakeLists.txt`) and would inherit
...
host-side orchestrator, the compiled binary does the computation on the device. The `_pops` module
