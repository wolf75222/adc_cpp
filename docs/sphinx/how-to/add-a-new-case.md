...
A *case* is a named scenario that builds a model, wires it into an `adc.System`, sets the
...
3. Write the case script in that folder. Compose a model the same way the tutorial does:
   `adc.Model(...)` for a native composition, `adc.dsl.Model(...)` for a formula model, or
   `adc.CompositeModel(...)` for a hybrid. See the [models overview](../models/index.md) for
   the three paths.
   ...
   `adc.AmrSystem` for adaptive refinement without any change.
