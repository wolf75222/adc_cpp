...
```bash
cmake -S . -B build -DCMAKE_BUILD_TYPE=Release -DPOPS_USE_KOKKOS=ON -DKokkos_ROOT="$KOKKOS_PREFIX"
```
...
`import pops`. There is no nvcc/CUDA Python module, so GPU runs stay C++-only, and
the Python module does not exercise the MPI code paths (MPI is validated through
the C++/ctest path). To distribute across ranks, add `-DPOPS_USE_MPI=ON` and see
[Parallel backends](../backends/index.md).
