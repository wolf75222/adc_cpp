...
|---------|-------|--------------|
| **Kokkos Serial** | `-DPOPS_USE_KOKKOS=ON`, `Kokkos_ENABLE_SERIAL=ON` | CI gate `build-and-test` (every PR), C++ + Python module. |
| **MPI + Kokkos Serial** | `-DPOPS_USE_MPI=ON -DPOPS_USE_KOKKOS=ON` | CI job MPI (`ci-full`), `mpirun` np=1/2/4. |
| **Kokkos OpenMP** | `-DPOPS_USE_KOKKOS=ON`, `Kokkos_ENABLE_OPENMP=ON` | CI job `ci-full` (enabled since #155). |
