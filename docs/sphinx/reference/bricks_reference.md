...
| `nx` / `time` / `n_blocks` / `n_patches` | -- | Diagnostics scalaires. `n_blocks()` = nombre de blocs ; `n_patches()` = nombre de patchs fins. |
...
| `patch_rectangles` | `patch_rectangles()` (recent) | Convertit `patch_boxes()` en rectangles physiques `(x0, y0, w, h)` dans `[0, L]^2` (un par patch fin). Pratique pour tracer les VRAIS patchs (p.ex. `matplotlib.Rectangle`). |

```{note}
`patch_boxes()` / `patch_rectangles()` exposent la GEOMETRIE REELLE des patchs fins (ajout
