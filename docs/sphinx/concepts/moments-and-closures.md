...
system is not closed on its own, how a closure repairs that, and what the `pops.lib.moments` generator
...
- **Gaussian (Levermore)** closure: assume the distribution is a local Maxwellian. The standardized
  ...
  flow really is Gaussian. `pops.lib.moments.gaussian_closure(order)` provides it.
...
From the closure alone, `pops.lib.moments.build_moment_model` produces a full
...
- [Moment models reference](../reference/moment-models.md) -- the `pops.lib.moments` API.
