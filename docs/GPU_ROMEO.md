...
rsync -az include /tmp/euler_gpu.cu python/tests/gpu/romeo_run.sh romeo:pops_dsl_gpu/
ssh romeo 'mv ~/pops_dsl_gpu/romeo_run.sh ~/pops_dsl_gpu/run.sh'

# 3. compiler (nvcc sm_90) + executer sur un noeud H100
ssh romeo 'cd ~/pops_dsl_gpu && srun --account=<compte> -p instant --constraint=armgpu \
...
euler_gpu && ./euler_gpu`. The `kflux` kernel instantiates `pops_generated::EulerGen` on the device, computes
the flux for a few states, and main compares against `pops::Euler` computed on the host.
...
BUILD_OK
device=NVIDIA GH200 120GB  maxdiff(GPU EulerGen vs hote pops::Euler)=0.000e+00
```
...
rsync -az include /tmp/kokkos_euler.cpp python/tests/gpu/kokkos_CMakeLists.txt \
      python/tests/gpu/romeo_kokkos_build.sh romeo:pops_dsl_kk/
ssh romeo 'cd ~/pops_dsl_kk && mkdir -p harness && mv kokkos_euler.cpp harness/ \
...
ssh romeo 'cd ~/pops_dsl_kk && srun --account=<compte> -p instant --constraint=armgpu \
...
KOKKOS_OK
HARNESS_OK
exec_space=Cuda  maxdiff(Kokkos EulerGen vs hote pops::Euler)=5.551e-17
```
...
rsync -az include /tmp/kokkos_euler_sim.cpp romeo:pops_dsl_kk/sim/   # + kokkos_sim_CMakeLists.txt -> sim/CMakeLists.txt
rsync -az python/tests/gpu/romeo_kokkos_sim_build.sh romeo:pops_dsl_kk/kk_sim_build.sh
ssh romeo 'cd ~/pops_dsl_kk && srun --account=<compte> -p instant --constraint=armgpu \
...
Result (obtained):
```
exec=Cuda  n=64 steps=80  mass_drel=0.000e+00  rho[min,max]=[0.8912,1.0464]  maxdiff(EulerGen vs pops::Euler, GPU)=8.882e-16
```
