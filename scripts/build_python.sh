#!/usr/bin/env bash
# One command to build + install the Python module `_adc` for END USERS, applying the build knobs that
...
#
#   - activates the conda env `adc` (override: ADC_ENV_NAME), without tripping `set -u`;
#   - sizes the heavy-TU Ninja pool (ADC_HEAVY_TU_POOL) from cores AND free RAM so the split module TUs
#     compile in PARALLEL without OOM (each -O3 leaf peaks at several GB; the CMake default is 1, the
#     CI out-of-memory guard). Pre-set ADC_HEAVY_TU_POOL to pin it by hand.
#   - exports the Kokkos / CMake discovery vars (Kokkos_ROOT, ADC_KOKKOS_ROOT, CMAKE_PREFIX_PATH) and a
...
#   bash scripts/build_python.sh --mpi      # also build the distributed MPI backend (ADC_USE_MPI=ON)
#   ADC_HEAVY_TU_POOL=4 bash scripts/build_python.sh    # pin the pool by hand (skip auto-sizing)
...
if [[ -n "${ADC_HEAVY_TU_POOL:-}" ]]; then
  pool="$ADC_HEAVY_TU_POOL"
  echo "heavy-TU pool: $pool (from ADC_HEAVY_TU_POOL)"
else
  ram_cap=$(( mem_gb / 4 )); [[ $ram_cap -lt 1 ]] && ram_cap=1
  pool=$ncpu; [[ $pool -gt $ram_cap ]] && pool=$ram_cap
  echo "heavy-TU pool: $pool (min of ${ncpu} cores and ${ram_cap} = ${mem_gb}GB/4; export ADC_HEAVY_TU_POOL to override)"
fi

# --- discovery vars + stable cross-worktree ccache --------------------------------------------------
export CMAKE_PREFIX_PATH="${CONDA_PREFIX}${CMAKE_PREFIX_PATH:+:$CMAKE_PREFIX_PATH}"
export Kokkos_ROOT="${Kokkos_ROOT:-$CONDA_PREFIX}"
export ADC_KOKKOS_ROOT="${ADC_KOKKOS_ROOT:-$CONDA_PREFIX}"
...
# --- build + install --------------------------------------------------------------------------------
[[ $WITH_MPI -eq 1 ]] && { export ADC_USE_MPI=ON; echo "MPI backend: ON"; }
cd "$HERE"
pip_args=(install -v . -C cmake.define.ADC_HEAVY_TU_POOL="$pool")
if python -c "import scikit_build_core, pybind11" >/dev/null 2>&1; then
  pip_args=(install -v . --no-build-isolation -C cmake.define.ADC_HEAVY_TU_POOL="$pool")
else
...
# --- diagnose ---------------------------------------------------------------------------------------
echo ""
echo "--- import adc; adc.doctor() ---"
python -c "import adc; print('adc', adc.__version__); adc.doctor()"
