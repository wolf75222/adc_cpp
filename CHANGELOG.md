...
- **Stale `condensed_schur` example expectation** (ADC-421 / ADC-427): `examples/time_programs/condensed_schur_program.py` `show_macro()` still expected `pops.lib.time.condensed_schur(theta != 1)` to raise `NotImplementedError`, so the example printed MISMATCH and exited 1 against the ADC-427 library, which lowers `theta` in `(0, 1]` (the `n+1` extrapolation by `1/theta`) and rejects only out-of-range `theta` with a `ValueError`. The example now asserts that `theta=1` and `theta=0.5` both lower the assemble/solve/reconstruct chain (the `theta=0.5` case carrying the `1/theta = 2.0` axpy) and that `theta=1.5` is rejected with `ValueError`, matching `python/tests/test_time_condensed_schur.py`. Docstrings updated; no library change.
