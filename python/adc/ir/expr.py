"""Symbolic expression node classes for the ADC IR (Spec 4).

This module owns the two PURE-symbolic node families, moved verbatim from
:mod:`adc.dsl` and :mod:`adc.math`. It carries NO C++ source emission.

  1. The flux-DSL expression tree: :class:`Expr` and its subclasses
     (:class:`Const`, :class:`Var`, :class:`_Bin`, :class:`Add`, :class:`Sub`,
     :class:`Mul`, :class:`Div`, :class:`Pow`, :class:`Neg`, :class:`Sqrt`,
     :class:`Abs`, :class:`Sign`), plus the :func:`_wrap` coercion helper.
  2. The board AST: :class:`Equation`, :class:`_BoardNode`, :class:`Partial`,
     :class:`Gradient`, :class:`Laplacian`, :class:`RateTerm`, :class:`RateExpr`,
     :class:`Divergence`, :class:`TimeDerivative`, :class:`Unknown`,
     :class:`OpApply`, :class:`Integral`, plus :func:`_as_rate`.

Witness / reference value nodes (:class:`EigWitness`, :class:`StateRef`,
:class:`RuntimeParamRef`) live in :mod:`adc.ir.values`. The ``to_cpp`` methods
kept on these nodes are pure string accessors carried verbatim; the codegen
walkers that consume them remain in :mod:`adc.dsl`.
"""
import numpy as np


# ===========================================================================
# Flux-DSL expression tree (from adc.dsl)
# ===========================================================================
...
# ===========================================================================
# Board AST node classes (from adc.math)
# ===========================================================================
