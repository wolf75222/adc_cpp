...
`adc.CompiledTime(substeps=, stride=)` records a SYSTEM-level cadence applied AROUND the opaque
...
Bit-exactness vs the native path holds ONLY on the simple cases the tests deliberately use:
  - substeps: a compiled Forward-Euler program over an UNCOUPLED / transport-only model matches
    native adc.Explicit(method="euler", substeps=n) bit-for-bit. (program_step_(h) re-runs the WHOLE
...
negative/zero raise. Section (B) (parity) needs _adc + a compiler + a visible Kokkos (ADC_KOKKOS_ROOT)
...
ct2 = adc.CompiledTime(substeps=2)
chk(ct2.substeps == 2 and ct2.stride == 1, "CompiledTime(substeps=2) stores substeps (no raise)")
cts = adc.CompiledTime(stride=2)
chk(cts.stride == 2 and cts.substeps == 1, "CompiledTime(stride=2) stores stride (no raise)")
chk(adc.CompiledTime(substeps=3, stride=4).substeps == 3
    and adc.CompiledTime(substeps=3, stride=4).stride == 4,
    "CompiledTime(substeps=3, stride=4) stores both")
chk(raises(ValueError, lambda: adc.CompiledTime(substeps=0)), "substeps=0 rejected (ValueError)")
chk(raises(ValueError, lambda: adc.CompiledTime(substeps=-1)), "substeps<0 rejected (ValueError)")
chk(raises(ValueError, lambda: adc.CompiledTime(stride=0)), "stride=0 rejected (ValueError)")
chk(raises(ValueError, lambda: adc.CompiledTime(stride=-2)), "stride<0 rejected (ValueError)")
...
# Skip cleanly without the install_program / set_program_cadence binding (rebuild _adc).
...
# ----- SUBSTEPS: compiled CompiledTime(substeps=2) vs native adc.Explicit(euler, substeps=2) -----
print("-- substeps --")
sub2, _ = run_compiled(adc.CompiledTime(substeps=2), 1)

nat = make_sim(adc.Explicit(method="euler", substeps=2))
nat.step(dt)
sub2_native = np.array(nat.get_state("ions"))
e_sub = float(np.abs(sub2 - sub2_native).max())
chk(e_sub < 1e-12, "compiled substeps=2 == native adc.Explicit(euler, substeps=2) (max|d|=%.2e)"
    % e_sub)
...
half_twice, _ = run_compiled(adc.CompiledTime(substeps=1), 2, dt_step=dt / 2.0)
...
sub1, _ = run_compiled(adc.CompiledTime(substeps=1), 1)
d_sub = float(np.abs(sub2 - sub1).max())
chk(d_sub > 1e-9, "substeps=2 differs from substeps=1 (non-degenerate, max|d|=%.2e)" % d_sub)

# ----- STRIDE: single-block system, compiled stride=2 vs native adc.Explicit(stride=2) -----
print("-- stride --")
K = 4  # macro-steps (even -> ends on a catch-up window boundary)
str2, t_str = run_compiled(adc.CompiledTime(stride=2), K)

nat_s = make_sim(adc.Explicit(method="euler", stride=2))
for _ in range(K):
    nat_s.step(dt)
str2_native = np.array(nat_s.get_state("ions"))
e_str = float(np.abs(str2 - str2_native).max())
chk(e_str < 1e-12, "compiled stride=2 == native adc.Explicit(euler, stride=2) over %d steps "
    "(max|d|=%.2e)" % (K, e_str))
...
str1, _ = run_compiled(adc.CompiledTime(stride=1), K)
