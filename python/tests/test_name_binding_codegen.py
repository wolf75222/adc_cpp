#!/usr/bin/env python3
"""adc.time NAME-based block binding codegen + IR identity (Spec 3 criterion 23, ADC-457).
...
(A) ABI export -- ``emit_cpp_program`` emits ``adc_program_block_count()`` returning N and an
    ``adc_program_block_name(int)`` switch returning each block's name in ``_block_indices`` order
...
Skips cleanly (never fakes the engine) if adc.time cannot import (it needs _adc for the typed registry).
"""
import sys
...
def section_a(t):
    print("== (A) the .so exports adc_program_block_name per block, declaration order ==")
    P = _flux_program(t, "plasma_electrons", ["plasma", "electrons", "dust"])
    src = P.emit_cpp_program()

    chk("adc_program_block_count() { return 3; }" in src,
        "adc_program_block_count returns the block count (3)")
    # Each block name appears at its declaration-order index in the adc_program_block_name switch.
    ...
    chk('extern "C" const char* adc_program_block_name(int i)' in src,
    ...
    chk("adc_program_block_count() { return 1; }" in src1, "single-block count is 1")
