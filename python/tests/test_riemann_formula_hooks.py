...
Capability-hook descriptors (``adc.lib.riemann.hllc.contact_speed.euler()``) and ``None`` keep
...
run need ADC_KOKKOS_ROOT (ROMEO) and are out of scope here.
"""
import re
...
def _pressure_body(cpp):
    ...
    mt = re.search(r"adc::Real pressure\(const State& U\) const \{(.*?)\n  \}", cpp, re.S)
...
def test_pressure_formula_overrides_role_derived_hook():
    ...
    assert "const adc::Real cs2 = " in body1            # the custom scalar appears as a local
...
def test_descriptor_hooks_keep_the_role_derived_default():
    ...
    assert "adc::Real contact_speed(const State& UL, const State& UR" in cpp
