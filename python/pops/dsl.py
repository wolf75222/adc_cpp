...
class HyperbolicModel:
    ...
    def emit_cpp(self, func=None, cse=True):
        ...
        nc = self.n_vars
        out = [
            "// genere depuis le modele symbolique '%s' (pops.dsl.emit_cpp)" % self.name,
    ...
    def emit_cpp_brick(self, name=None, namespace="pops_generated", cse=True,
        ...
        def cons_locals():
            return ["    const pops::Real %s = U[%d];" % (c, i) for i, c in enumerate(self.cons_names)]
        ...
        S += ["    const pops::Real %s = P[%d];" % (p, i) for i, p in enumerate(self.prim_state)]
    ...
    def emit_cpp_elliptic(self, name=None, namespace="pops_generated", cse=True,
        ...
        out += ["    const pops::Real %s = U[%d];" % (c, i) for i, c in enumerate(self.cons_names)]
    ...
    def emit_cpp_elliptic_field(self, field, struct_name, namespace="pops_generated",
        ...
        out += ["namespace %s {" % namespace, "struct %s {" % struct_name,
                ...
                "  using State = pops::StateVec<%d>;" % self.n_vars]
        if rt_member:
            out.append(rt_member.rstrip("\n"))
